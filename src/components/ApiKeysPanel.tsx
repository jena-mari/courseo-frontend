...
function friendlyKeyError(error: unknown) {
  ...
  if (/invalid|rejected|check it/i.test(message)) {
    return "This API key appears to be invalid. Check that you copied the full key and try again.";
  }
  if (/session|log in/i.test(message)) return message;
  if (/temporarily|server|reach|connection|respond/i.test(message)) {
    return "Courseo can’t verify your key right now. Check your connection and try again.";
  }
  return "We couldn’t connect this API key. Check it and try again.";
}
...
export function ApiKeysPanel({ compact = false, onConnected }: { compact?: boolean; onConnected?: () => void }) {
  ...
  const load = useCallback(async (showLoading = true) => {
    ...
    } catch (error) { setMessageTone("error"); setMessage(friendlyKeyError(error)); }
  ...
  const add = async () => {
    ...
    setBusy("add"); setMessage(""); setMessageTone("neutral");
    ...
    try {
      ...
      await load(false); setLabel(""); setMessageTone("success"); setMessage("API key verified and saved securely."); onConnected?.();
    } catch (error) { setMessageTone("error"); setMessage(friendlyKeyError(error)); }
    finally { setSecret(""); setBusy(""); }
  };
  const verify = async (id: string) => { setBusy(id); try { await verifyApiKey(id); await load(false); setMessageTone("success"); setMessage("API key verified and ready to use."); onConnected?.(); } catch (error) { setMessageTone("error"); setMessage(friendlyKeyError(error)); } finally { setBusy(""); } };
  const remove = async (id: string) => { if (!window.confirm("Remove this API key? Existing chats may stop working.")) return; setBusy(id); try { await deleteApiKey(id); await load(false); setMessageTone("neutral"); setMessage("API key removed."); onConnected?.(); } catch (error) { setMessageTone("error"); setMessage(friendlyKeyError(error)); } finally { setBusy(""); } };
  const makeDefault = async (id: string) => { setBusy(id); try { await updateApiKey(id, { make_default: true }); await load(false); setMessageTone("success"); setMessage("Default key updated."); onConnected?.(); } catch (error) { setMessageTone("error"); setMessage(friendlyKeyError(error)); } finally { setBusy(""); } };
  const saveEdit = async (id: string) => {
    ...
    try { await updateApiKey(id, changes); await load(false); setEditingId(""); setEditLabel(""); setMessageTone("success"); setMessage("API key details updated."); onConnected?.(); }
    catch (error) { setMessageTone("error"); setMessage(friendlyKeyError(error)); }
    finally { setBusy(""); }
  };
