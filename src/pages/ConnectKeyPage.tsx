...
import { getKeyProviders, getSavedKeys, usableProviderModels, type ProvidersResponse, type SavedCredential } from "../lib/keyApi";
...
export function ConnectKeyPage() {
  ...
  return <div className="relative h-[100dvh] w-full overflow-y-auto px-4 py-6 font-['Montserrat',sans-serif]">
    ...
    <main className="relative z-10 mx-auto my-auto w-full max-w-[720px] rounded-[30px] bg-white px-6 py-8 shadow-[0_28px_80px_rgba(0,0,0,0.32)] sm:px-10">
      ...
      {loading ? <div className="mt-8 space-y-4" role="status" aria-live="polite"><div className="flex items-center justify-center gap-3 text-[13px] font-extrabold text-[#000181]"><LoaderCircle className="animate-spin" size={19} /> Preparing secure provider setup…</div><div className="space-y-3 rounded-[16px] border border-[rgba(0,1,129,0.1)] p-4"><div className="grid gap-3 sm:grid-cols-2"><div className="h-11 animate-pulse rounded-[13px] bg-[#eef0ff]" /><div className="h-11 animate-pulse rounded-[13px] bg-[#f6efff]" /></div><div className="h-11 animate-pulse rounded-[13px] bg-[#eef0ff]" /></div></div> : error ? <p role="alert" className="mt-6 text-center text-sm font-semibold text-red-600">{error}</p> : <>
