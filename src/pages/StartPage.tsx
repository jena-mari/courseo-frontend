  ...
  return <motion.section initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.09 } } }} className="relative mx-auto grid w-[calc(100vw-2rem)] min-w-0 max-w-[1240px] items-center gap-x-10 gap-y-3 sm:w-[calc(100vw-3.5rem)] lg:w-full lg:grid-cols-[1.08fr_0.92fr]">
    <div className="relative z-10 min-w-0 pt-2 text-center lg:pt-0 lg:text-left">
      <div className="overflow-hidden">
        ...
        <motion.h1 variants={{ hidden: { opacity: 0, y: 70 }, show: { opacity: 1, y: 0 } }} transition={{ duration: 0.75, ease: [0.16, 1, 0.3, 1] }} className="courseo-hero-word mt-[-0.04em] text-[clamp(64px,12vw,150px)] font-black leading-[0.88] tracking-[-0.075em]">Courseo</motion.h1>
      </div>
      <motion.p variants={{ hidden: { opacity: 0, y: 18 }, show: { opacity: 1, y: 0 } }} transition={{ duration: 0.5 }} className="mx-auto mt-7 max-w-[540px] text-[15px] font-semibold leading-relaxed text-[rgba(0,1,129,0.68)] lg:mx-0 lg:text-[17px]">Turn your enrolment record into a handbook-aware study plan, then ask questions and refine every session with Courseo.</motion.p>
    ...
    <motion.div variants={{ hidden: { opacity: 0, y: 26 }, show: { opacity: 1, y: 0 } }} transition={{ duration: 0.55 }} className="relative z-20 mt-7 grid min-w-0 gap-4 lg:col-span-2 lg:mt-9 lg:grid-cols-[1.35fr_0.85fr]">
      <button type="button" onClick={onCourseo} disabled={loading} aria-busy={loading} className="group relative flex min-h-[138px] w-full min-w-0 items-center justify-between overflow-hidden rounded-[25px] border-2 border-[#000181] bg-[#000181] px-4 py-5 text-left text-white shadow-[0_18px_42px_rgba(0,1,129,0.24)] transition duration-300 hover:-translate-y-1 hover:bg-[#171899] hover:shadow-[0_22px_48px_rgba(0,1,129,0.3)] disabled:cursor-wait sm:px-7">
        <span className="relative flex min-w-0 flex-1 items-center gap-3 sm:gap-5"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[17px] bg-white shadow-lg sm:h-16 sm:w-16 sm:rounded-[19px]"><img src={imgLogo} alt="" className="h-10 w-10 object-contain sm:h-11 sm:w-11" /></span><span className="min-w-0"><span className="mb-2 inline-flex rounded-full bg-[#83e7ff] px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-[#000181]">Recommended</span><strong className="block text-[17px] font-black leading-tight text-white sm:text-[21px]">{loading ? "Checking your session…" : "Proceed to Courseo"}</strong><span className="mt-1 block text-[10px] font-semibold leading-relaxed text-white/75 sm:text-[12px]">{user ? "Open your handbook-aware planner and saved chats." : "Create an account for personalised plans and saved chats."}</span></span></span><span className="relative ml-2 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-[#000181] shadow-lg transition-transform group-hover:translate-x-1 sm:ml-3 sm:h-12 sm:w-12"><ArrowRight size={20} /></span>
      </button>
      <button type="button" onClick={onCopilot} className="group flex min-h-[138px] w-full min-w-0 items-center justify-between rounded-[25px] border-2 border-[rgba(0,1,129,0.22)] bg-white px-4 py-5 text-left shadow-[0_14px_34px_rgba(0,1,129,0.12)] transition duration-300 hover:-translate-y-1 hover:border-[#000181] hover:shadow-[0_18px_40px_rgba(0,1,129,0.18)] sm:px-6">
